...
const BookList = () => {
  ...
  const handleDelete = (bookNumber) => {
    ...
    console.log("Delete book", bookNumber);
  };

  const handleUpdate = (bookNumber) => {
    // Handle update action
    console.log("Update book", bookNumber);
  };
          ...
          <tr className="pb-2">
            <th colSpan="10">
              <div className="flex justify-between items-center px-5 py-4">
                <h2 className="text-2xl text-black">Book list</h2>
                <button
  ...
  {filteredData.map((item) => (
    <tr key={item.BookNumber} className="border-b border-gray">
      ...
      <td className={`px-5 py-2 ${item.Status === "Available" ? "text-green" : "text-red"}`}>
        {item.Status}
      </td>
      <td className="px-5">
        <Dropdown options={rowActions(item.BookNumber)} />
      </td>
    </tr>
  ))}
</tbody>
